...
if (isset($_POST['scEvAdd-submit'])) {
    ...
    if (empty($eventD) || empty($startT) || empty($endT) || empty($title) || empty($descrip)) {
        ...
        header("Location: ../pages/mainPage.php?error=emptyfields");
        exit();
    } else {
        // And finally we put in the values into the database (saftly)
        $sql = "INSERT INTO events (eDate, eStart, eEnd, eTitle, eDes, uID) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = mysqli_stmt_init($conn);
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            header("Location: ../pages/mainPage.php?error=sqlerror2");
            exit();
        } else {
            mysqli_stmt_bind_param($stmt, "ssssss", $eventD, $startT, $endT, $title, $descrip, $_SESSION['userID']);
            mysqli_stmt_execute($stmt);
            header("Location: ../pages/mainPage.php?book=success");
            exit();
        }
    }
